...
SELECT E1_FILIAL AS EMPRESA,RTRIM(M0_FILIAL) AS NOMEEMP,RTRIM(E1_NUM) AS NUMERO,RTRIM(E1_PARCELA) AS PARCELA, RTRIM(E1_TIPO) AS TIPO,E1_PREFIXO AS SERIE,E1_VALOR AS VALOR,E1_SALDO AS SALDO, RTRIM(E1_CODBAR) CODBARRAS,RTRIM(E1_CODDIG) AS LINDIG,RTRIM(E1_NUMBCO) AS NUMBCO,RTRIM(E1_NUMBOR) AS BORDERO,
...
CASE E1_TIPO WHEN 'BOL'  THEN 'Boleto' WHEN 'FT' THEN 'Faturado' WHEN 'NP' THEN 'Nota Promissória' WHEN 'REN' THEN 'Renegociação'
 WHEN 'DP' THEN 'Depósito' WHEN 'CC' THEN 'Cartão de Crédito' WHEN 'CD' THEN 'Cartão de Débito' WHEN 'PX' THEN 'Pix' WHEN 'CH' THEN 'Cheque' WHEN 'R$' THEN 'Dinheiro' 
 ELSE 'Negociado' END AS TIPODESC
...
WHERE E1_TIPO IN ('BOL','NP','REN','DP','CC','CD','FT','PX','CH','R$')